...
describe("scrubText", () => {
  ...
  it("scrubs the path out of a file:// URL — a document name is user data", () => {
    const out = scrubText(
      "Error loading file:///Users/kirby/Desktop/Q3 salary review.md",
      "/Users/kirby"
    );
    expect(out).not.toContain("/Users/kirby");
    expect(out).not.toContain("Desktop");
    // The bug still needs to be findable: the basename may stay.
  });

  it("replaces the short username only as a whole word", () => {
    const out = scrubText("markdown parse failed for mark", "/Users/mark");
    // "markdown" must survive; the bare name must not.
