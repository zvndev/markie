...
// already was.
//
// A copy of src/lib/rehype-media.ts, for the same reason render.ts is a copy
// of markdown-html.ts: the server must not depend on the Next app's module
// graph. Both are covered by tests that assert the same behaviour, so a change
// to one that is not made to the other fails.
