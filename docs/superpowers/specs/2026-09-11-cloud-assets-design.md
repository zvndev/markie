...
| `PUT /api/docs/:id/assets` `{ refs: [{ ref, hash? }] }` | owner or editor | Replaces the document's full reference set. An entry with a hash must name an asset in the caller's scope, and that asset's stored mime must be the one `assetMimeFor` gives for the reference, so a reference whose extension names no type cannot carry a hash at all; an entry that disagrees is dropped and named in `droppedRefs`, never stored. An entry without a hash keeps the existing link for that ref if there is one (an editor pushing text whose images the owner uploaded), otherwise it is dropped. Refuses a set whose total exceeds 500 MB, and one longer than 2000 entries with 413 `{ error: "too many references", cap }`. Refuses the whole body with 400 `{ error: "bad ref" }` when any ref is empty, over 2048 bytes or carries a control character, and, for every caller but the owner of a document nobody else can reach, when a ref that carries a hash lands outside the document's folder (one with no hash is dropped instead, and never kept). Outside means an absolute `/` or `\`, a drive or UNC form, or segments that resolve above the folder with `.` dropped and `..` popping, so `./x.png` and `a/../x.png` are inside and `../x.png` and `a/../../x.png` are not. Returns `{ linked, kept, dropped, droppedRefs }`, where `droppedRefs` names the entries the server itself decided against with a reason: `"mime"` for one whose stored type is not the type its reference names, which is dropped rather than refused because an asset row's type is written once and a rename must not cost a document its other pictures, and `"escaping"` for a bare reference outside the folder. Rows no longer referenced by any document are removed from `assets` and from storage. |
...
- miss: `GET /api/docs/:id/assets/file?ref=` through `sync.js`'s authenticated `api()` helper, stream to a temp file while hashing and counting it, and file it under the `ETag` hash only when the bytes actually hash to that name; a mismatch (a lying origin, or a download that was cut short) is discarded and serves nothing. The size recorded in the index is the count that was written, never the declared `Content-Length`, because that number is what the 2 GB bound is computed from.
