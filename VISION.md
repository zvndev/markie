...
## What this is
Markie is a native macOS markdown workspace: fast editor, live rich view, device-wide markdown
index, sharing, collaboration, comments, theme sync, and a built-in MCP surface for local AI agents.
It should feel like the Google Docs of Markdown while keeping the user's files on their Mac and
making cloud features optional.
...
A user can install Markie on Apple Silicon macOS, open or find a markdown file, edit it in View or
source mode, save/export/share it, and use agent tooling around it without account friction. A
feature is done only when its user journey is reproduced and the relevant renderer, Electron, MCP,
or server tests pass.
